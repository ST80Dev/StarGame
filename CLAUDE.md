...
22. **Modalità di gioco — modello multi-pista con tracker (decisione architetturale per M05 + M20).** Per aumentare rigiocabilità e offrire partite di durata scalabile (20-150 min), il gioco supporta **8 piste di vittoria tematiche** (Sandbox · Esploratore · Colonizzatore · Egemone economico · Ascensione tecnologica · Pacifista/Federatore · Tiranno/Sovrano oscuro · Sopravvissuto). Scelte utente:
    - **Implementazione progressiva** — nessun big-bang. **Tier 0** al day-one di M20 (Sandbox/Esploratore/Colonizzatore, già supportabili coi moduli core M02-M08); **Tier 1** post-M11/M12 (Egemone economico, Pacifista); **Tier 2** post-M13/M17/M19 (Ascensione tech, Tiranno, Sopravvissuto). L'infrastruttura (game state, `victoryCheck`, scheduler) va però predisposta **in M05** indipendentemente da quando ciascuna pista diventa vincibile.
    - **Poche azioni nuove per le modalità "morali"** — Tiranno e Pacifista sono *principalmente win condition*, non set d'azioni paralleli. Massimo 2-3 verbi extra per lato (Tiranno: saccheggio/propaganda/schiavismo, ganci su M09/M11/M19; Pacifista: patto federativo/mediazione/aiuto umanitario, ganci su M11/M12). I verbi ricevono un marcatore `alignmentImpact: 'dark' | 'light' | null` che alimenta le piste Reputazione (+/-).
    - **Multi-pista in parallelo** — ogni partita ha tutte le 8 piste attive contemporaneamente con `score: 0..1`. La modalità scelta all'avvio (`startedAs`) è solo **enfasi narrativa** (tutorial, eventi iniziali, AI di partenza), non un lock. Vince la **prima pista chiusa**. Vincere su una pista diversa dalla `startedAs` è un *pivot* premiato narrativamente. Tracker "Vie di vittoria" sempre visibile nel pannello *Consiglio* con barre per pista; notifiche di avvicinamento in Cronaca Galattica alle soglie 50%/75%/90%.
    - **Sopravvissuto come evento alla DS 0, non modifica galassia** — la crisi §17 della modalità Sopravvissuto viene **iniettata sul delta** all'avvio partita (lo scheduler eventi di M17 deve supportare `triggerAt: 'gameStart'`). La generazione galassia resta **immutabile** dal seed: lo stesso seed è giocabile in *qualunque* modalità (determinismo seed+delta, decisione #5, preservato).
    - **Preset curati + sblocco libero opzionale** — la schermata "Nuova partita" (M20) mostra **3-4 preset** (*Classico* · *Speedrun* · *Incubo* · *Lungo respiro*) che impacchettano modalità + 4 modificatori trasversali (**dimensione galassia** small/standard/large · **pericolo §5.3** standard/hostile · **velocità TSG** ×1/×2/×4 · **Ironman** off/on). Un toggle "Personalizza" sblocca i modificatori liberamente con avviso "alcune combinazioni possono risultare squilibrate". In Ironman: un solo slot save, export/import disabilitato, no undo.
    - **Impatto sul data model (M05/M06)**: bump `schemaVersion: 1 → 2` con migrazione (default `sandbox`/`classic`/modifiers default); nuovi slot `mode: { startedAs, preset, modifiers }` e `victoryTracks: { [track]: { score, available, won } }`; hook puro `victoryCheck(state) → Array<TrackResult>` chiamato dal game loop ogni `victoryTickInterval` Impulsi (default 5); modificatore TSG agisce solo sul timer reale del loop (non sulla logica).
    - **Dettaglio completo in `MODALITA_GIOCO.md`** (root del repo, 13 sezioni): catalogo piste, modello multi-pista, modificatori, preset, tabella di responsabilità per ognuno dei 20 moduli, data model, game loop, UI/UX, verbi con allineamento, determinismo, roadmap Tier 0→2, glossario. Quel documento è la fonte di dettaglio: questa decisione ne è il sigillo architetturale.
...
- **Creare GDD §16 Vittoria** prima di M20 (oggi non esiste sezione vittoria nel GDD): le 8 piste tematiche, il modello multi-pista, il tracker, le modalità di partenza e gli eventi narrativi associati. Riferimento di dettaglio già pronto in `MODALITA_GIOCO.md` (decisione #22).
- **Definire i 3-4 preset concreti** (*Classico · Speedrun · Incubo · Lungo respiro*) con numeri e testi finali quando arriveremo a **M20**: combinazione di modalità + 4 modificatori (dimensione galassia, pericolo §5.3, velocità TSG, Ironman), durata target, descrizione narrativa per la schermata Nuova partita.
- **Adeguare il lavoro già prodotto per M05** alla decisione #22 (slot `mode`/`victoryTracks` nello stato, `victoryCheck` multi-pista, supporto eventi `triggerAt: 'gameStart'`, modificatore TSG, marcatore `alignmentImpact` nelle action). Messaggio di handoff già preparato per la sessione di M05.
...
├── README.md            ✅
├── MODALITA_GIOCO.md    ✅ riferimento decisione #22 (modalità di gioco + impatto sui moduli)
...
_Aggiornamento successivo (2026-06-04, post M04): **decisione #22 — modalità di gioco multi-pista**. Formalizzate le 8 piste di vittoria tematiche (Sandbox · Esploratore · Colonizzatore · Egemone economico · Ascensione tech · Pacifista · Tiranno · Sopravvissuto), il modello multi-pista con tracker visibile, i 4 modificatori trasversali (dimensione galassia / pericolo §5.3 / velocità TSG / Ironman) e i 3-4 preset curati. Implementazione **progressiva** in tre tier (day-one M20 → post-M11/M12 → post-M13/M17/M19); infrastruttura (bump `schemaVersion: 2`, `victoryCheck`, scheduler eventi `triggerAt:'gameStart'`, marcatore `alignmentImpact`) da predisporre in **M05** indipendentemente da quando le piste diventano vincibili. Riferimento di dettaglio: `MODALITA_GIOCO.md` (13 sezioni, tabella di responsabilità per ognuno dei 20 moduli, data model, game loop, UI/UX, roadmap). Promemoria aggiunti in *Problemi aperti*: creare GDD §16 Vittoria, definire i preset numerici in M20, adeguare il codice di M05 già prodotto._
